...
//版本控制、区别于所有的、使将每一个模块拥有独立的版本，可以进行选择性或者强制性的模块的版本更新与下载，拥有统一的版本管理查看下载卸载更新界面
//比如：
//如果现在有N个模块、其中会有一个版本控制的界面，这个界面是“实时”的，这里可以获取到关于所有版本的所有的信息供用户选择性下载更新。
//对于其中的一个模块，可以拥有独立于不同于其他模块的版本控制，结构等版本管理系统
//例如：
//      1:将一个功能模块分为若干个小模块去管理：UI？模型？预制体？然后可以去更新替换他们其中的每一部分，比如可以在统一（部分）去自定义的更新UI贴图，甚至是支持自定义贴图或者用户自主上传分享下载，去拓展自己的UI风格。
//      2:将一个功能模块分成若干个小模块，每次热更管理只替换更新其中的一部分即可实现功能的整体替换。
//      3:甚至是替换不同的脚本、已实现不同的展示效果或者功能性能上的不同。也可在版本管理工具中强制更新到最少哪个版本才能继续使用这个功能。
