...
#endif
            Type tIFlatbufferObject = typeof(FlatBuffers.IFlatbufferObject);
            //Type tIMessageSerializer = typeof(IMessageSerializer);

            foreach (var t in check_types)
            {
                //if (((t.IsEnum || tIFlatbufferObject.IsAssignableFrom(t)) && t.Namespace == "msg_fbs") ||
                  //  t.IsSubclassOf(tIMessageSerializer))
                if ((t.IsEnum || tIFlatbufferObject.IsAssignableFrom(t)) && t.Namespace == "msg_fbs")
                {
                    LuaCallCSharpAttribute ccla = new LuaCallCSharpAttribute();
                    AddToList(LuaCallCSharp, () => t, ccla);
                }
